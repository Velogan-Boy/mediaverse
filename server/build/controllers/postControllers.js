...
exports.getUserFeed = catchAsync(async (req, res) => {
   ...
   let posts = await PostModel.find().sort('-createdAt').populate({
      path: 'userid',
...
exports.getTrendingPosts = catchAsync(async (req, res) => {
   ...
   for (const item of trendingHashtags) {
      const trendingPost = await PostModel.find({ hashtags: { $in: item._id } }).sort('-upvotes').populate('userid').limit(1);
